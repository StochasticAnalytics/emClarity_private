...
if strcmpi(STAGEofALIGNMENT, 'RawAlignment')
  ...
  elseif strcmpi(postOperation, 'RemoveClasses')
    % Folded 2026-07-21: `geometry ... RemoveClasses` now writes the culled geometry straight to
    % RawAlign itself, so there is no separate skip step and no Post_RemoveClasses to promote.
    error(['skip RemoveClasses is obsolete: the class cull now writes RawAlign directly.\n', ...
           '       Run "emClarity geometry <param> <cycle> Cluster_cls RemoveClasses <mod> STD" ', ...
           'and do NOT follow it with a skip.']);
  else
    % Plain skip (no postOperation) or explicit 'Override': promote the un-culled geometry to RawAlign.
