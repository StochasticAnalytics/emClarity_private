...
def test_debug_file_behavior():
    ...
    try:
        ...
        instrument_click_event(
            "test_button_1", "QPushButton", "First test click", "test_panel", 
            force_log=True
        )
        
        instrument_click_event(
            "test_button_2", "QPushButton", "Second test click", "test_panel", 
            force_log=True
        )
        
        instrument_click_event(
            "test_button_3", "QPushButton", "Third test click", "test_panel", 
            force_log=True
        )
        ...
        if debug_files:
            ...
            if isinstance(data, list):
                ...
                for i, event in enumerate(data, 1):
                    print(f"   Event {i}: {event['element_id']} - {event['action_description']}")
                ...
                print(f"❌ Expected list of events, got single event: {data['element_id']}")
        else:
...
def test_ctrl_behavior():
    ...
    try:
        ...
        instrument_click_event(
            "forced_click", "QPushButton", "Forced click test", "test_panel",
            force_log=True
        )
