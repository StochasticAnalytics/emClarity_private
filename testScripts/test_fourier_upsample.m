...
% Mirror-pad
mirror_x = [window(end:-1:2, :); window; window(end-1:-1:2, :)];
mirror_xy = [mirror_x(:, end:-1:2), mirror_x, mirror_x(:, end-1:-1:2)];
...
% Find peak
[peak_height, up_idx] = max(upsampled(:));
[up_mx, up_my] = ind2sub(size(upsampled), up_idx);

% Convert back to original coordinates
mirror_offset_x = win_size - 2;
mirror_offset_y = win_size - 2;

orig_region_start_x = mirror_offset_x * upsample_factor + 1;
orig_region_start_y = mirror_offset_y * upsample_factor + 1;

dx_subpix = (up_mx - orig_region_start_x - hw * upsample_factor) / upsample_factor;
dy_subpix = (up_my - orig_region_start_y - hw * upsample_factor) / upsample_factor;
