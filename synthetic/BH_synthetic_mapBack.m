...
function local_write_tomoCPR_sweep_script(fname)
  ...
  C = {
    ...
    'new_peak_found=0'
    ''
    'function require_leave_out {'
    '  # Pull tiltalign''s leave-out cross-validation line for scoring. A whiff here'
    '  # is a real solve failure OR tiltalign''s log format drifted (e.g. an IMOD'
    '  # version bump renamed the line); fail fast either way, but first echo the'
    '  # actual leave-out lines so the cause is visible instead of a bare exit.'
    '  local log=$1'
    '  local label=$2'
    '  local matched'
    '  matched=$(grep -i "$label" "$log") || matched=""'
    '  if [[ -z $matched ]]; then'
    '    echo "-- leave-out lines present in $log (tiltalign actual format): --" >&2'
    '    grep -i "leave-out" "$log" >&2 || tail -n 25 "$log" >&2'
    '    echo "ERROR: score grep matched no $label line in $log; tiltalign cross-validation format may have changed (e.g. IMOD version bump). Update the score grep in this script." >&2'
    '    exit 1'
    '  fi'
    '  leave_out_line=$matched'
    '}'
    ''
    'function run_align {'
    ''
    ...
    '$do_robust'
    'EOF'
    '# Score only during the parameter search. The final RobustFitting pass just'
    '# writes the outputs; its robust-format log would miss the grep and pipefail.'
    'if [[ -z $do_robust ]]; then'
    'if [[ -z $do_local ]]; then'
    '  require_leave_out "$output_log_file" "global leave-out"'
    '  score=$(echo "$leave_out_line" | awk ''{print $6}'')'
    ...
    '    fi'
    'else'
    '  require_leave_out "$output_log_file" "local leave-out"'
    '  score=$(echo "$leave_out_line" | awk ''{print $6}'')'
