...
class GPUContext:
    ...
    def __init__(self):
        ...
        if self.has_gpu:
            try:
                ...
                logger.info(f"CUDA available with {self.device_count} device(s), using device {self.current_device}")
    ...
    def ensure_array_type(self, array: Any, prefer_gpu: bool = False) -> Tuple[Any, Any]:
        """
    ...
    def set_device(self, device_id: int):
        ...
        if self.has_gpu and 0 <= device_id < self.device_count:
            ...
            logger.warning(f"Cannot set device {device_id} (available: 0-{self.device_count-1})")


def validate_array_dimensions(array: Any, expected_dims: int, name: str = "array") -> None:
    """Validate array has expected number of dimensions."""
    if array.ndim != expected_dims:
        raise ValueError(f"{name} must be {expected_dims}D, got {array.ndim}D array with shape {array.shape}")


def validate_array_shape_compatibility(arr1: Any, arr2: Any, operation: str = "operation") -> None:
    """Validate two arrays have compatible shapes for element-wise operations."""
    if arr1.shape != arr2.shape:
        raise ValueError(f"Array shapes incompatible for {operation}: {arr1.shape} vs {arr2.shape}")


def validate_array_dtype(array: Any, expected_dtypes: Union[type, Tuple[type, ...]], name: str = "array") -> None:
...
def safe_file_path(path: Union[str, Path], must_exist: bool = True, create_parent: bool = False) -> Path:
    """
...
def setup_logging(level: int = logging.INFO, format_string: Optional[str] = None, 
                 include_cuda_info: bool = True) -> None:
    ...
    if include_cuda_info:
        ...
        if HAS_CUPY:
            try:
                ...
                logger.info(f"CUDA initialized: {device_count} device(s), current device: {current_device}")
            except Exception as e:
