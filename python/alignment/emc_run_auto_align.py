...
from image_io.mrc_image import MRCImage, OPEN_IMG, SAVE_IMG
...
def emc_run_auto_align(
    ...
    binning_params = _calculate_binning_parameters(auto_ali_params, emc['pixel_size_angstroms'])
    ...
    if switch_axes:
        raise NotImplementedError("Axis switching requires GPU functions - not yet implemented")
    ...
    preprocessed_stack = _preprocess_stack(input_stack, emc, nx, ny, nz, auto_ali_params)
    ...
    _run_integrated_patch_tracking(
        paths, rot_mat, emc, img_rotation, 
        binning_params, nx, ny, nz, auto_ali_params, fixed_params
    )
...
def _handle_skip_tilts(skip_tilts: Optional[List[int]], total_tilts: int) -> Optional[List[bool]]:
...
def _process_tilt_angles(tilt_angles: Union[str, Path], skip_tilts_logical: List[bool], base_name: str) -> None:
...
def _calculate_binning_parameters(auto_ali_params: dict, pixel_size_angstroms: float) -> dict:
    ...
    if auto_ali_params['max_sampling_rate'] > 4:
        ...
        bin_low = int(np.ceil(auto_ali_params['max_sampling_rate'] / pixel_size_angstroms))
    
    bin_inc = -1 * int(np.ceil((bin_high - bin_low) / 3))
    
    # Calculate shift limits
    first_iter_shift_limit_pixels = int(np.ceil(
        auto_ali_params['max_shift_angstroms'] / pixel_size_angstroms
    ))
...
def _should_switch_axes(emc: dict, input_stack: np.ndarray, img_rotation: float) -> bool:
    """Check if axes should be switched (placeholder - requires GPU functions)."""
    if emc.get('autoAli_switchAxes', False):
        logger.warning("autoAli_switchAxes requires GPU functions - not yet implemented")
...
def _save_modified_stack(input_mrc: MRCImage, paths: dict, header_info: dict) -> None:
    ...
    SAVE_IMG(input_mrc, f'fixedStacks/{paths["base_name"]}.fixed', 
...
def _create_stack_symlink(stack_in: Union[str, Path], paths: dict) -> None:
    ...
    os.chdir('fixedStacks')
    try:
        subprocess.run(['ln', '-sf', f'../{stack_in}', f'{paths["base_name"]}.fixed'], 
                      check=True)
...
def _preprocess_stack(input_stack: np.ndarray, emc: dict, nx: int, ny: int, nz: int, 
                     auto_ali_params: dict) -> np.ndarray:
    """
    ...
    """
    logger.warning("Using simplified preprocessing - full implementation requires BH_bandpass3d")
...
def _save_preprocessed_stack(preprocessed_stack: np.ndarray, paths: dict, emc: dict, 
...
def _run_integrated_patch_tracking(paths: dict, rot_mat: List[float],
                                  emc: dict, img_rotation: float, binning_params: dict,
                                  nx: int, ny: int, nz: int, auto_ali_params: dict, 
    ...
    with open('preRotXf.xf', 'w') as f:
        f.write(f'{rot_mat[0]:.3f} {rot_mat[1]:.3f} {rot_mat[2]:.3f} {rot_mat[3]:.3f} 0.0 0.0\n')
    ...
    for i_bin in bin_sequence:
        ...
        lp_cut = i_bin * emc['pixel_size_angstroms'] / auto_ali_params['resolution_cutoff']
        ...
        for bin_iter in range(1, auto_ali_params['iterations_per_bin'] + 1):
            ...
            # Calculate shift limits
            if iteration >= 2:
                limit_shifts = int(binning_params['first_iter_shift_limit_pixels'] / 
                                 (iteration ** auto_ali_params['divide_shift_limit_by'])) + 1
            else:
            ...
            try:
                if iteration == 1:
                    ...
                    _run_subsequent_iteration_alignment(
                        p_name, paths, iteration, i_bin, lp_cut, hp_cut,
                        limit_shifts, pt_size_x, pt_size_y, pt_border, pt_overlap,
                        tilt_angle_offset, img_rotation, auto_ali_params, 
                        tilt_option, mag_option
                    )
                ...
                raise RuntimeError(f"Patch tracking failed at iteration {iteration}: {e}")
            finally:
...
def _run_first_iteration_alignment(p_name: str, paths: dict, limit_shifts: int, 
    ...
    cmd = [
        'tiltxcorr',
        '-InputFile', f'../../fixedStacks/{paths["base_name"]}.fixed.preprocessed',
        '-OutputFile', f'{p_name}.prexf',
        '-TiltFile', f'../../fixedStacks/{paths["base_name"]}.rawtlt',
        '-FilterSigma1', '0.001',
        '-FilterRadius2', '0.5',
        '-FilterSigma2', '0.05',
        '-CumulativeCorrelation',
        '-ShiftLimitsXandY', f'{limit_shifts},{limit_shifts}',
        '-AngleOffset', str(tilt_angle_offset)
    ]
...
def _run_subsequent_iteration_alignment(p_name: str, paths: dict, iteration: int, 
                                      i_bin: int, lp_cut: float, hp_cut: float,
                                      limit_shifts: int, pt_size_x: int, pt_size_y: int,
                                      pt_border: int, pt_overlap: float,
                                      tilt_angle_offset: float, img_rotation: float,
                                      auto_ali_params: dict, tilt_option: int, 
                                      mag_option: int) -> None:
    """Run subsequent iteration alignment with patch tracking."""
    import shutil
    
    # Setup file links
    shutil.copy(f'../../fixedStacks/{paths["base_name"]}.fixed.preprocessed', f'{p_name}.st')
    ...
    if iteration == 2:
        ...
        cmd = [
            'xfproduct',
            '-InputFile1', f'{prev_w_dir}/INP-{prev_iter}.inpXF',
            '-InputFile2', f'{prev_w_dir}/INP-{prev_iter}.tltxf',
            '-OutputFile', f'{p_name}.inpXF'
        ]
        try:
    ...
    cmd = [
        'newstack',
        '-InputFile', f'{p_name}.st',
        '-OutputFile', f'{p_name}.preali',
        '-TransformFile', f'{p_name}.inpXF',
        '-BinByFactor', str(i_bin),
        '-AntialiasFilter', '5',
        '-FloatDensities', '2'
    ]
    ...
    cmd = [
        'tiltxcorr',
        '-OverlapOfPatchesXandY', f'{pt_overlap},{pt_overlap}',
        '-IterateCorrelations', '1',
        '-ImagesAreBinned', str(i_bin),
        '-InputFile', f'{p_name}.preali',
        '-OutputFile', f'{p_name}_pt.fid',
        '-PrealignmentTransformFile', f'{p_name}.prexg',
        '-TiltFile', f'{p_name}.rawtlt',
        '-FilterSigma1', str(hp_cut),
        '-FilterRadius2', str(lp_cut),
        '-FilterSigma2', '0.05',
        '-ShiftLimitsXandY', f'{limit_shifts},{limit_shifts}',
        '-PadsInXandY', '128,128',
        '-SizeOfPatchesXandY', f'{pt_size_x},{pt_size_y}',
        '-BordersInXandY', f'{pt_border},{pt_border}',
    ...
    cmd = [
        'imodchopconts',
        '-InputModel', f'{p_name}_pt.fid',
        '-OutputModel', f'{p_name}.fid',
        '-NumberOfPieces', '2'
    ]
    ...
    # Run tiltalign
    _run_tiltalign(p_name, i_bin, local_rotation, tilt_angle_offset, iteration,
                   auto_ali_params, tilt_option, mag_option, pt_size_x, pt_size_y)


def _run_tiltalign(p_name: str, i_bin: int, local_rotation: float, 
                  tilt_angle_offset: float, iteration: int, auto_ali_params: dict,
                  tilt_option: int, mag_option: int, pt_size_x: int, pt_size_y: int) -> None:
    """Run tiltalign for fiducial-based alignment."""
    
    base_cmd = [
        'tiltalign',
        '-ModelFile', f'{p_name}.fid',
        '-ImageFile', f'{p_name}.preali',
        '-ImagesAreBinned', str(i_bin),
        '-OutputModelFile', f'{p_name}.3dmod',
        '-OutputResidualFile', f'{p_name}.resid',
        '-OutputFidXYZFile', f'{p_name}fid.xyz',
        '-OutputTiltFile', f'{p_name}.tlt',
        '-OutputTransformFile', f'{p_name}.tltxf_nonScaled',
        '-RotationAngle', str(local_rotation),
        '-TiltFile', f'{p_name}.rawtlt',
        '-AngleOffset', str(tilt_angle_offset),
        '-RotOption', '1',
        '-RotDefaultGrouping', '3',
        '-TiltOption', str(tilt_option),
        '-TiltDefaultGrouping', '3',
        '-MagOption', str(mag_option),
    ...
    if iteration > auto_ali_params['n_iters_no_rot']:
        base_cmd.extend([
            '-LocalAlignments',
            '-OutputLocalFile', f'{p_name}_local.xf',
            '-TargetPatchSizeXandY', f'{pt_size_x},{pt_size_y}',
    ...
    try:
        with open('tiltAlign.log', 'w') as log_file:
            subprocess.run(base_cmd, stdout=log_file, stderr=subprocess.STDOUT, 
...
def _copy_final_alignment_results(paths: dict, final_round: int) -> None:
    ...
    try:
        ...
        shutil.copy(f'round_{final_round}/INP-{final_round}_local.xf', 
                   f'../fixedStacks/{base_name}.local')
        
        # Combine transforms
        cmd = [
            'xfproduct',
            '-InputFile1', f'round_{final_round}/INP-{final_round}.inpXF',
            '-InputFile2', f'round_{final_round}/INP-{final_round}.tltxf',
            '-OutputFile', 'withOutRot.xf'
        ]
        subprocess.run(cmd, check=True, timeout=120)
        
        # Copy final transforms and tilt angles
        shutil.copy('withOutRot.xf', f'../fixedStacks/{base_name}.xf')
        shutil.copy(f'round_{final_round}/INP-{final_round}.tlt', 
...
def _run_integrated_bead_finding(paths: dict, nx: int, ny: int, emc: dict) -> None:
    ...
    try:
        ...
        logger.info(f"Bead finding parameters: size={bead_size_pixels}px, thickness={thickness}")
        
        # Step 1: Create binned aligned stack
        cmd = [
            'newstack',
            '-InputFile', f'{base_name}.fixed.preprocessed',
            '-OutputFile', f'{base_name}_3dfind.ali',
            '-TransformFile', f'{base_name}.xf',
            '-ImagesAreBinned', '1.0',
            '-BinByFactor', '15'
        ]
        ...
        cmd = [
            'tilt',
            '-InputProjections', f'{base_name}_3dfind.ali',
            '-OutputFile', f'{base_name}_3dfind.rec',
            '-IMAGEBINNED', '15',
            '-TILTFILE', f'{base_name}.tlt',
            '-LOCALFILE', f'{base_name}.local',
        ...
        cmd = [
            'findbeads3d',
            '-InputFile', f'{base_name}_3dfind.rec',
            '-OutputFile', f'{base_name}_3dfind.mod',
            '-BeadSize', str(bead_size_pixels),
        ...
        try:
            ...
            logger.warning(f"findbeads3d failed: {e} - this may be expected if no beads found")
        
        # Step 4: Create erased reconstruction (project beads out)
        cmd = [
            'tilt',
            '-InputProjections', f'{base_name}_3dfind.ali',
            '-OutputFile', f'{base_name}.erase',
            '-IMAGEBINNED', '15',
            '-TILTFILE', f'{base_name}.tlt',
            '-LOCALFILE', f'{base_name}.local',
            '-THICKNESS', str(thickness),
            ...
            '-OFFSET', '0.0',
            '-SHIFT', '0.0,0.0',
            '-ProjectModel', f'{base_name}_3dfind.mod',
            '-FULLIMAGE', f'{nx},{ny}',
            '-PERPENDICULAR',
            '-MODE', '2'
        ]
...
def _handle_tilt_options(tilt_option: int, base_name: str) -> None:
    ...
    if str(tilt_option) == '0':
        ...
        try:
            subprocess.run(['cp', f'fixedStacks/{base_name}.rawtlt', 
                          f'fixedStacks/{base_name}.tlt'], check=True)
        except subprocess.CalledProcessError as e:
            logger.warning(f"Failed to copy tilt angles: {e}")


def _run_bead_refinement(refine_on_beads: bool, paths: dict, nx: int, ny: int, 
                        emc: dict) -> bool:
    """
    ...
    try:
        ...
        for ext_item in ext_list:
            subprocess.run(['ln', '-sf', f'../fixedStacks/{paths["base_name"]}.{ext_item}', 
                          f'{paths["base_name"]}.{ext_item}'], check=True)
        
        subprocess.run(['ln', '-sf', f'../fixedStacks/{paths["base_name"]}.fixed.preprocessed', 
                       f'{paths["base_name"]}.fixed'], check=True)
        
        # Run bead refinement
        # TODO: Implement BH_refine_on_beads when available
        logger.warning("Bead refinement requires BH_refine_on_beads - using placeholder")
        min_sampling_rate = 5
        ...
        if not too_few_beads:
            ...
            logger.warning("Too few beads found. Using iterative patch tracking results")
...
def main():
    ...
    parser.add_argument("--skip-tilts", help="Comma-separated list of tilt indices to skip")
