...
class CudaBasicOps:
    ...
    def _load_cuda_kernels(self):
        ...
        try:
            ...
            self._cuda_kernels = {
                ...
                'multiply_scalar': self._cuda_module.get_function('cuda_multiply_scalar'),
    ...
    def _calculate_grid_block(self, total_threads: int, block_size: int = 256) -> Tuple[int, int]:
        """Calculate grid and block dimensions for 1D kernels."""
        blocks = (total_threads + block_size - 1) // block_size
        return (blocks,), (block_size,)
    
    def _calculate_grid_block_2d(self, rows: int, cols: int, 
                                 block_x: int = 16, block_y: int = 16) -> Tuple[Tuple[int, int], Tuple[int, int]]:
    ...
    def _calculate_grid_block_3d(self, nx: int, ny: int, nz: int,
                                 block_x: int = 8, block_y: int = 8, block_z: int = 8) -> Tuple[Tuple[int, int, int], Tuple[int, int, int]]:
    ...
    def add_arrays(self, a: cp.ndarray, b: cp.ndarray) -> cp.ndarray:
        ...
        self._cuda_kernels['add_arrays'](
            grid, block,
            (a.data.ptr, b.data.ptr, c.data.ptr, n)
        )
    ...
    def multiply_scalar(self, a: cp.ndarray, scalar: float) -> cp.ndarray:
        ...
        self._cuda_kernels['multiply_scalar'](
            grid, block,
            (a.data.ptr, b.data.ptr, cp.float32(scalar), n)
        )
    ...
    def transpose_2d(self, a: cp.ndarray) -> cp.ndarray:
        ...
        self._cuda_kernels['transpose_2d'](
            grid, block,
            (a.data.ptr, out.data.ptr, rows, cols)
        )
    ...
    def transpose_3d(self, a: cp.ndarray) -> cp.ndarray:
        ...
        self._cuda_kernels['transpose_3d'](
            grid, block,
            (a.data.ptr, out.data.ptr, nx, ny, nz)
        )
