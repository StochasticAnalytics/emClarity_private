...
class BasicArrayOps:
    ...
    def _load_cuda_kernels(self):
        ...
        try:
            ...
            self._kernels['transpose_2d'] = self._cuda_module.get_function('transpose_2d')
            self._kernels['transpose_3d_xy'] = self._cuda_module.get_function('transpose_3d_xy')
            self._kernels['vector_magnitude_squared'] = self._cuda_module.get_function('vector_magnitude_squared')
    ...
    def _calculate_grid_size(self, n_elements: int, block_size: int = 256) -> Tuple[int, int]:
        """
    ...
    def _calculate_2d_grid_size(self, shape: Tuple[int, int], 
                               block_size: Tuple[int, int] = (16, 16)) -> Tuple[Tuple[int, int], Tuple[int, int]]:
        """
    ...
    def _calculate_3d_grid_size(self, shape: Tuple[int, int, int],
                               block_size: Tuple[int, int, int] = (8, 8, 8)) -> Tuple[Tuple[int, int, int], Tuple[int, int, int]]:
        """
    ...
    def array_add(self, a: cp.ndarray, b: cp.ndarray, 
                  output: Optional[cp.ndarray] = None) -> cp.ndarray:
        """
        ...
        if output is None:
            ...
            raise ValueError(f"Output shape {output.shape} doesn't match input {a.shape}")
        ...
        self._kernels['array_add'](
            ...
            (a, b, output, n_elements)
        )
        
        return output
    
    def array_scale(self, input_array: cp.ndarray, scale_factor: float,
                   output: Optional[cp.ndarray] = None) -> cp.ndarray:
        """
        ...
        if output is None:
            ...
            raise ValueError(f"Output shape {output.shape} doesn't match input {input_array.shape}")
    ...
    def transpose_2d(self, input_array: cp.ndarray,
                    output: Optional[cp.ndarray] = None) -> cp.ndarray:
        """
        ...
        if output is None:
            ...
            raise ValueError(f"Output shape {output.shape} doesn't match expected {(cols, rows)}")
        ...
        self._kernels['transpose_2d'](
            grid_size, block_size,
            (input_array, output, rows, cols)
        )
        
        return output
    
    def transpose_3d_xy(self, input_array: cp.ndarray,
                       output: Optional[cp.ndarray] = None) -> cp.ndarray:
        """
        ...
        if output is None:
            ...
            raise ValueError(f"Output shape {output.shape} doesn't match expected {(ny, nx, nz)}")
        ...
        self._kernels['transpose_3d_xy'](
            grid_size, block_size,
            (input_array, output, nx, ny, nz)
        )
        
        return output
    
    def vector_magnitude_squared(self, input_array: cp.ndarray,
                                output: Optional[cp.ndarray] = None) -> cp.ndarray:
        """
