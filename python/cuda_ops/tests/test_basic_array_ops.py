...
try:
    from basic_array_ops import BasicArrayOps, cuda_array_add, cuda_array_scale, cuda_transpose_2d, cuda_transpose_3d_xy
    CUDA_AVAILABLE = True
...
class TestBasicArrayOps(unittest.TestCase):
    ...
    def test_array_add_small(self):
        ...
        np.testing.assert_allclose(
            cp.asnumpy(result_cuda), 
            cp.asnumpy(result_reference),
            rtol=1e-6, atol=1e-6
        )
    ...
    def test_convenience_functions(self):
        ...
        # Compare
        np.testing.assert_allclose(cp.asnumpy(result_add), cp.asnumpy(ref_add), rtol=1e-6)
        np.testing.assert_allclose(cp.asnumpy(result_scale), cp.asnumpy(ref_scale), rtol=1e-6)
