    ...
    def setup_ui(self):
        ...
        if self.parameter.param_type == "vector":
            ...
            for i in range(vector_size):
                if (
                    isinstance(self.parameter.default, (list, tuple))
                    and len(self.parameter.default) > i
                ):
                ...
                if (
                    hasattr(self.parameter, "vector_bounds")
                    and self.parameter.vector_bounds
                    and i < len(self.parameter.vector_bounds)
                ):
        ...
        if self.parameter.description:
            ...
            desc_label.setStyleSheet(
                "color: #333; font-size: 14pt; font-weight: normal; margin-top: 3px;"
            )
        ...
        if self.parameter.param_type == "choice" and self.parameter.choices:
            if self.parameter.name == "symmetry":
                ...
                tooltip_parts.append(
                    "Common symmetries: C1 (no symmetry), C2 (2-fold), C12 (12-fold), D2 (dihedral), O (octahedral)"
                )
                tooltip_parts.append(
                    f"All choices: {', '.join(self.parameter.choices)}"
                )
            else:
        ...
        if save_path:
            try:
                ...
                QMessageBox.information(
                    self, "Success", f"Parameters saved to {save_path}"
                )
...
class EnhancedTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.commands_content_layout.setSpacing(
            2
        )  # Tighter spacing for collapsed panels
        ...
        placeholder = QLabel(
            "Parameter configuration will appear here when tab is populated."
        )
    ...
    def add_command_panel(self, panel):
        ...
        if hasattr(panel, "expansion_changed"):
            ...
            collapsible_panel = CollapsibleCommandPanel(
                panel.command_name, panel.command_description, panel.parameters, self
            )
            # Connect signals
