...
class ProfileValidator(QThread):
    ...
    validation_result = Signal(str, str, bool, str)  # profile_name, resource_id, is_valid, message
    debug_output = Signal(str)  # debug message
    debug_output_end = Signal()  # signal to add spacing after method

    def __init__(self, profile_name: str, resource_id: str, hostname: str, username: str, gpus: int, project_path: str):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            self.debug_output.emit(f"Starting validation for {self.username}@{self.hostname}")
            ...
            process = subprocess.run(ssh_command, shell=True, capture_output=True, text=True, timeout=10)
            ...
            if process.returncode != 0:
                self.validation_result.emit(self.profile_name, self.resource_id, False, f"SSH connection failed: {process.stderr.strip() if process.stderr else 'Connection refused'}")
                return
            ...
            if process.returncode != 0:
                self.validation_result.emit(self.profile_name, self.resource_id, False, f"Project directory not found on remote host.")
                return
            ...
            if process.returncode != 0:
                self.validation_result.emit(self.profile_name, self.resource_id, False, "Project directory is not writable on remote host.")
                return
            ...
            process = subprocess.run(gpu_check_command, shell=True, capture_output=True, text=True, timeout=10)
            ...
            if process.returncode != 0:
                self.validation_result.emit(self.profile_name, self.resource_id, True, "Could not verify GPU count (nvidia-smi not found?).")
                return
            
            try:
                remote_gpus = int(process.stdout.strip())
                self.debug_output.emit(f"Found {remote_gpus} GPUs, expected {self.gpus}")
                if remote_gpus < self.gpus:
                    self.validation_result.emit(self.profile_name, self.resource_id, False, f"Expected {self.gpus} GPUs, but found {remote_gpus}.")
                    return
            except (ValueError, IndexError) as e:
                self.debug_output.emit(f"Error parsing GPU count: {e}")
                self.validation_result.emit(self.profile_name, self.resource_id, True, "Could not parse GPU count from remote host.")
                return

            self.debug_output.emit("Validation completed successfully")
            self.validation_result.emit(self.profile_name, self.resource_id, True, "Profile validated successfully.")
            self.debug_output_end.emit()

        except subprocess.TimeoutExpired:
            self.debug_output.emit("Validation timed out")
            self.validation_result.emit(self.profile_name, self.resource_id, False, "Validation timed out.")
            self.debug_output_end.emit()
        except Exception as e:
            self.debug_output.emit(f"Validation exception: {e}")
            self.validation_result.emit(self.profile_name, self.resource_id, False, f"An error occurred during validation: {e}")
            self.debug_output_end.emit()
...
class RunProfileWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.profiles: Dict[str, List[Dict[str, Any]]] = {}  # profile_name -> list of resources
    ...
    def setup_ui(self):
        ...
        self.tree.setHeaderLabels(["Profile/Resource", "Hostname", "Username", "GPUs", "Status"])
    ...
    def on_tree_selection_changed(self):
        ...
        if current_item:
            ...
            if parent:  # This is a resource item
                ...
                self.gpus_input.setValue(int(current_item.text(3)) if current_item.text(3).isdigit() else 0)
    ...
    def add_resource_to_profile(self):
        ...
        for resource in self.profiles[profile_name]:
            if resource["hostname"] == hostname and resource["username"] == username:
                QMessageBox.warning(self, "Duplicate Resource", "This resource already exists in the profile.")
                return
        ...
        self.validator_thread = ProfileValidator(profile_name, resource_id, hostname, username, gpus, project_path)
        ...
        if hasattr(self.parent_window, 'debug_output'):
            ...
            self.validator_thread.debug_output_end.connect(self.parent_window.debug_output_end_method)
    ...
    def update_selected_resource(self):
        ...
        current_item = self.tree.currentItem()
        if not current_item:
            QMessageBox.warning(self, "No Selection", "Please select a resource to update.")
            return

        parent = current_item.parent()
        if not parent:
            QMessageBox.warning(self, "Invalid Selection", "Please select a resource (not a profile) to update.")
            return
        ...
        for resource in self.profiles[profile_name]:
            if resource["hostname"] == old_hostname and resource["username"] == old_username:
    ...
    def copy_profile(self):
        ...
        if not new_profile_name or new_profile_name == source_profile_name:
            QMessageBox.warning(self, "Invalid Name", "Please enter a new profile name in the Profile field.")
            return
        ...
        self.update_profile_combo()
        self.save_profiles()
        QMessageBox.information(self, "Profile Copied", f"Profile '{source_profile_name}' copied to '{new_profile_name}' with {len(self.profiles[new_profile_name])} resources.")

    def delete_selected(self):
        """Delete the selected profile or resource."""
        current_item = self.tree.currentItem()
        if not current_item:
            QMessageBox.warning(self, "No Selection", "Please select a profile or resource to delete.")
            return
        ...
        if parent:  # Deleting a resource
            ...
            if reply == QMessageBox.Yes:
                # Remove the resource
                self.profiles[profile_name] = [r for r in self.profiles[profile_name] 
    ...
    def update_tree(self):
        ...
        for profile_name, resources in self.profiles.items():
            profile_item = QTreeWidgetItem([profile_name, "", "", f"{len(resources)} resources", ""])
    ...
    def revalidate_selected(self):
        ...
        if selected_item.parent():  # Resource item
            ...
            if hasattr(self.parent_window, 'debug_output'):
                self.parent_window.debug_output(f"Selected resource: {profile_name} -> {resource_id}")
            ...
            if profile_name not in self.profiles or not self.profiles[profile_name]:
                if hasattr(self.parent_window, 'debug_output'):
                    self.parent_window.debug_output(f"Profile {profile_name} has no resources")
                return
        ...
        if not resource_data:
            if hasattr(self.parent_window, 'debug_output'):
                self.parent_window.debug_output(f"Resource data not found for {resource_id}")
            return
        
        if hasattr(self.parent_window, 'debug_output'):
            self.parent_window.debug_output(f"Found resource data: {resource_data['hostname']}@{resource_data['username']}")
