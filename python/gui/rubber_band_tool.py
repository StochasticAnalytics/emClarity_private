...
class GuiContextAnalyzer:
        ...
        try:
            ...
            if hasattr(self.main_window, "sidebar_widget"):
                ...
                panel_info = {
                    ...
                    "panel_class": (
                        str(type(sidebar.panels.get(current_panel, None)).__name__)
                        if hasattr(sidebar, "panels")
                        else "unknown"
                    ),
                }
                ...
                if hasattr(sidebar, "panels") and current_panel in sidebar.panels:
                    ...
                    panel_info["object_name"] = getattr(
                        panel_widget, "objectName", lambda: ""
                    )()
            ...
            if hasattr(self.main_window, "sidebar_widget"):
                ...
                if current_panel and hasattr(sidebar, "panels"):
                    ...
                    if panel_widget:
                        ...
                        if panel_module:
                            context["active_panel_file"] = getattr(
                                panel_module, "__file__", "unknown"
                            )
    ...
    def analyze_layout(self, selection_rect: QRect) -> Dict[str, Any]:
        ...
        return {
            ...
            "corners": {
                "top_left": {
                    "x": selection_rect.topLeft().x(),
                    "y": selection_rect.topLeft().y(),
                },
                "bottom_right": {
                    "x": selection_rect.bottomRight().x(),
                    "y": selection_rect.bottomRight().y(),
                },
            },
        }
...
class GuiElementInfoDialog(QDialog):
    ...
    def __init__(
        self, selection_rect: QRect, context_analysis: Dict[str, Any], parent=None
    ):
        super().__init__(parent)
    ...
    def _generate_enhanced_prompt(self, coords_text: str, size_text: str) -> str:
        ...
        if (
            click_data_list
            and isinstance(click_data_list, list)
            and len(click_data_list) > 0
        ):
...
"""
        ...
        pos_text = (
            f"({click_pos.get('x', 0):.0f}, {click_pos.get('y', 0):.0f})"
            if click_pos
            else "unknown"
        )
...
"""
    ...
    def _format_widgets_summary(self, widgets: List[Dict[str, Any]]) -> str:
        ...
        for widget in widgets[:5]:  # Limit to first 5 widgets
            ...
            summary.append(f"- {widget_type}{text_part}")
...
class RubberBandTool:
    ...
    def activate(self):
        ...
        self.overlay.escape_pressed.connect(
            self.deactivate
        )  # Direct connection to deactivate
    ...
    def handle_selection(self, rect: QRect):
        ...
        widget_count = len(
            [
                w
                for w in context_analysis.get("widgets_in_selection", [])
                if "error" not in w
            ]
        )
        ...
        print(
            f"🎯 UI Type: {visual_info.get('likely_ui_type', 'unknown').replace('_', ' ')}"
        )
