...
class GUIStateManager:
    ...
    def init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if "resource_id" not in columns:
                ...
                for row in old_data:
                    # Convert old format: (id, profile_name, hostname, username, gpus, status,
                    # timestamp)
                    profile_name, hostname, username, gpus, status = (
                        row[1],
                        row[2],
                        row[3],
                        row[4],
    ...
    def save_tab_state(self, tab_name: str, active_tab: bool, expanded_panels: list):
    ...
    def load_tab_state(self, tab_name: str) -> Optional[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if row:
                ...
                return {"active_tab": bool(row[0]), "expanded_panels": expanded_panels}
        return None
    ...
    def import_state(self, import_path: str):
        ...
        for project in state.get("recent_projects", []):
            self.save_recent_project(
                project["name"], project["path"], project.get("parameter_file")
            )
    ...
    def load_run_profiles(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT profile_name, resource_id, hostname, username, gpus, status FROM run_profiles"
            )
            for row in cursor.fetchall():
                profile_name = row[0]
