    ...
    def _log_click_event(self, widget, event):
        ...
        try:
            ...
            click_data = {
                ...
                "click_position": {"x": event.position().x(), "y": event.position().y()},
                "global_position": {"x": event.globalPosition().x(), "y": event.globalPosition().y()}
            }
            ...
            # Print to console
            print(f"🎯 Click logged: {widget_info['class']} '{widget_info['text']}' in {widget_info['parent']}")
...
def init_rubber_band_debug(enabled: bool = False):
    ...
    if enabled and QT_AVAILABLE:
        ...
        print(f"🔧 INIT: Not initializing debug - enabled={enabled}, QT_AVAILABLE={QT_AVAILABLE}")
        _DEBUG_DATA_FILE = None
...
INCLUDE_DEBUG_INSTRUMENTATION = os.getenv('EMCLARITY_DEBUG_INSTRUMENTATION', '1').lower() in ['true', '1', 'yes', 'on']

if not INCLUDE_DEBUG_INSTRUMENTATION:
    # Replace all functions with no-ops for production builds
    def init_rubber_band_debug(*args, **kwargs): pass
    def toggle_click_logging(): return False
    def setup_click_logging_shortcut(*args, **kwargs): return None
    def is_rubber_band_debug_active(): return False
    def is_click_logging_enabled(): return False
    def get_latest_click_data(): return None
    def clear_click_data(): pass
