...
class PaddedArray:
    ...
    def _initialize_stored_array(self):
        ...
        logger.debug(
            f"Initialized {self.method} padded array with shape {self.output_shape}"
        )
    ...
    def pad_image(
        ...
        use_stored = (
            not self.use_once
            and not force_new_array
            and self._array_is_initialized
            and self.output_shape == output_shape
        )
        ...
        self._apply_padding_operations(
            padded_img, image, pad_low_vals, pad_top_vals, fourier_oversample, is_2d
        )
    ...
    def _apply_padding_operations(
        self, padded_img, image, pad_low_vals, pad_top_vals, fourier_oversample, is_2d
    ):
        ...
        if fourier_oversample:
            ...
            self._place_image_standard(
                padded_img, image, pad_low_vals, pad_top_vals, is_2d
            )
    ...
    def _place_image_center(self, padded_img, image, is_2d):
        ...
        if is_2d:
            padded_img[start_idx[0] : end_idx[0], start_idx[1] : end_idx[1], 0] = image
        else:
            padded_img[
                start_idx[0] : end_idx[0],
                start_idx[1] : end_idx[1],
...
def create_padded_array_once(
    ...
    """
    padder = PaddedArray(
        method=method, precision=precision, use_once=True, extrap_val=extrap_val
    )
