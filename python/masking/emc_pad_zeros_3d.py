...
def emc_pad_zeros_3d(
    ...
    if method == "GPU" and not HAS_CUPY:
        raise RuntimeError("GPU method requested but CuPy is not available. "
    ...
    if use_taper:
        image = _apply_tapering(image, extrap_val if extrap_val is not None else 0, is_2d)
...
def _apply_tapering(image, extrap_val, is_2d):
    ...
    if len(image.shape) == 3 or not is_2d:
        # 3D tapering
        # X dimension edges
        image[:7, :, :] = (image[:7, :, :] * taper[:, None, None] + 
                          extrap_val * inv_taper[:, None, None])
        image[-7:, :, :] = (image[-7:, :, :] * taper[::-1, None, None] + 
                           extrap_val * inv_taper[::-1, None, None])
        
        # Y dimension edges  
        image[:, :7, :] = (image[:, :7, :] * taper[None, :, None] + 
                          extrap_val * inv_taper[None, :, None])
        image[:, -7:, :] = (image[:, -7:, :] * taper[None, ::-1, None] + 
        ...
        if not is_2d and image.shape[2] > 7:
            image[:, :, :7] = (image[:, :, :7] * taper[None, None, :] + 
                              extrap_val * inv_taper[None, None, :])
            image[:, :, -7:] = (image[:, :, -7:] * taper[None, None, ::-1] + 
                               extrap_val * inv_taper[None, None, ::-1])
    else:
        # 2D tapering
        image[:7, :] = image[:7, :] * taper[:, None] + extrap_val * inv_taper[:, None]
        image[-7:, :] = image[-7:, :] * taper[::-1, None] + extrap_val * inv_taper[::-1, None]
        image[:, :7] = image[:, :7] * taper[None, :] + extrap_val * inv_taper[None, :]
        image[:, -7:] = image[:, -7:] * taper[None, ::-1] + extrap_val * inv_taper[None, ::-1]
