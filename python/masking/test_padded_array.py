...
from padded_array import PaddedArray, create_padded_array_once, BH_padZeros3d_class
...
def test_different_padding_modes():
    ...
    for mode_name, extrap_val in modes:
        ...
        if extrap_val == "random":
            padding_stats = f"mean={np.mean(padding_region):.3f}, std={np.std(padding_region):.3f}"
        else:
...
def benchmark_vs_original():
    ...
    avg_persistent_time = np.mean(persistent_times[1:])  # Exclude first (includes allocation)
