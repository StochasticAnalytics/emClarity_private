...
class MRCImage:
    ...
    def __init__(self, filename: Optional[Union[str, Path]] = None, 
                 flg_load: bool = False, debug: bool = False):
        """
    ...
    def open(self, filename: Union[str, Path], flg_load: bool = False, 
        ...
        if debug:
            ...
            logger.info(f"Dimensions: {self.get_nx()} x {self.get_ny()} x {self.get_nz()}")
    ...
    def _load_header(self) -> None:
        ...
        with mrcfile.open(self._filename, mode='r', permissive=True) as mrc:
            ...
            if self._header['mx'] > 0:
                self._header['pixel_size_x'] = self._header['cell_a'] / self._header['mx']
            if self._header['my'] > 0:
                self._header['pixel_size_y'] = self._header['cell_b'] / self._header['my']
            if self._header['mz'] > 0:
                self._header['pixel_size_z'] = self._header['cell_c'] / self._header['mz']
    ...
    def _load_data(self) -> None:
        ...
        if self._debug:
            logger.info(f"Loaded data: shape={self._data.shape}, dtype={self._data.dtype}")
    ...
    @classmethod
    def from_header(cls, header_dict: Dict[str, Any], 
                    filename: Optional[Union[str, Path]] = None) -> 'MRCImage':
        """
...
def SAVE_IMG(mrc_image: MRCImage, filename: Union[str, Path], 
             pixel_size=None, origin=None) -> None:
    """
...
def create_test_mrc(filename: Union[str, Path], shape: Tuple[int, int, int] = (10, 20, 30), 
...
def test_mrc_image_basic():
    ...
    try:
        ...
        print(f"   Cell dimensions: {header['cell_a']:.1f} x {header['cell_b']:.1f} x {header['cell_c']:.1f}")
        ...
        print(f"   Loaded back - shape: {data_loaded.shape}, dtype: {data_loaded.dtype}")
