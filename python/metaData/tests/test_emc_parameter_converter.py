    ...
    def test_example_conversion(self):
        ...
        for matlab_name, matlab_value in matlab_params.items():
            if matlab_name in self.converter.parameter_mapping:
                ...
                if param_info.param_type == float and matlab_name in ['PIXEL_SIZE', 'Cs', 'VOLTAGE', 'max_ctf3dDepth', 'defEstimate']:
                    converted_value = self.converter._convert_units_matlab_to_json(matlab_name, matlab_value)
                    expected_value = expected_json_values[param_info.new_name]
                    self.assertAlmostEqual(converted_value, expected_value, places=6,
...
def create_sample_json_config():
    ...
    sample_config = {
        ...
        "sample": {
            "fiducial_diameter_angstroms": 100.0
        },
        "tomocpr": {
