...
class EmClarityMetadataConverter:
    ...
    def _define_geometry_columns(self) -> List[str]:
        ...
        return [
            ...
            'reserved_2', 'reserved_3',   # Columns 2-3
            'subtomo_index',              # Column 4 - unique particle ID
            'reserved_5', 'reserved_6',   # Columns 5-6
            'half_set',                   # Column 7 - odd/even half set (1/2)
            'reserved_8', 'reserved_9',   # Columns 8-9
            'reserved_10',                # Column 10
            'pos_x', 'pos_y', 'pos_z',   # Columns 11-13 - position in tomogram
            'euler_psi', 'euler_theta', 'euler_phi',  # Columns 14-16 - Euler angles
            'rot_matrix_r1c1', 'rot_matrix_r1c2', 'rot_matrix_r1c3',  # Columns 17-19
            'rot_matrix_r2c1', 'rot_matrix_r2c2', 'rot_matrix_r2c3',  # Columns 20-22  
            'rot_matrix_r3c1', 'rot_matrix_r3c2', 'rot_matrix_r3c3',  # Columns 23-25
    ...
    def _define_tilt_geometry_columns(self) -> List[str]:
        ...
        return [
            ...
            'reserved_5', 'reserved_6', 'reserved_7', 'reserved_8',  # Columns 5-8
            'reserved_9', 'reserved_10', 'reserved_11', 'reserved_12',  # Columns 9-12
            'reserved_13', 'reserved_14', 'reserved_15', 'reserved_16',  # Columns 13-16
            'reserved_17', 'reserved_18', 'reserved_19', 'reserved_20',  # Columns 17-20
            'reserved_21', 'reserved_22', 'reserved_23'  # Columns 21-23
        ]

    def convert_mat_to_star(self, mat_file_path: Union[str, Path], 
    ...
    def _convert_top_level_metadata(self, subtomo_meta: Any, output_dir: Path) -> None:
        ...
        scalar_fields = [
            'currentCycle', 'currentTomoCPR', 'currentResForDefocusError',
            'maxGoldStandard', 'nSubTomoInitial', 'CUTPADDING'
        ]
    ...
    def _convert_geometry_data(self, subtomo_meta: Any, output_dir: Path) -> None:
        ...
        for cycle_field in cycle_fields:
            ...
            if hasattr(cycle_data, 'geometry'):
                self._convert_cycle_geometry(cycle_data.geometry, cycle_field, geometry_dir)
            elif hasattr(cycle_data, 'Avg_geometry'):
                self._convert_cycle_geometry(cycle_data.Avg_geometry, f"{cycle_field}_avg", geometry_dir)

    def _convert_cycle_geometry(self, geometry_data: Any, cycle_name: str, 
        ...
        if hasattr(geometry_data, '_fieldnames'):
            ...
            tomo_names = [attr for attr in dir(geometry_data) 
                         if not attr.startswith('_') and 
                         hasattr(geometry_data, attr)]
        ...
        if all_geometry_data:
            ...
            logger.info(f"Saved {len(combined_df)} particles from {len(all_geometry_data)} tomograms to {output_file}")

    def _array_to_geometry_dataframe(self, array: np.ndarray, tomo_name: str) -> pd.DataFrame:
        ...
        if n_cols == len(self.geometry_columns):
            ...
            logger.warning(f"Unexpected number of columns ({n_cols}) for geometry data, using generic names")
    ...
    def _convert_tilt_geometry(self, subtomo_meta: Any, output_dir: Path) -> None:
        ...
        if hasattr(tilt_geometry, '_fieldnames'):
            ...
            tilt_names = [attr for attr in dir(tilt_geometry) 
    ...
    def _array_to_tilt_dataframe(self, array: np.ndarray, tilt_name: str) -> pd.DataFrame:
        ...
        if n_cols == len(self.tilt_geometry_columns):
            ...
            # Generate generic column names
            columns = [f'col_{i+1}' for i in range(n_cols)]
            logger.warning(f"Unexpected number of columns ({n_cols}) for tilt geometry, using generic names")
    ...
    def _convert_tomo_coordinates(self, tomo_coords: Any, mapback_dir: Path) -> None:
        ...
        for tomo_name in tomo_names:
            try:
                ...
                coord_fields = ['is_active', 'y_i', 'y_f', 'NX', 'NY', 'NZ',
                               'dX_specimen_to_tomo', 'dY_specimen_to_tomo', 'dZ_specimen_to_tomo',
                               'tilt_NX', 'tilt_NY']
        ...
        if coord_data:
            ...
            starfile.write(df, output_file)
            logger.info(f"Saved coordinates for {len(coord_data)} tomograms to {output_file}")
    ...
    def _convert_tomo_names(self, tomo_names: Any, mapback_dir: Path) -> None:
        ...
        if name_data:
            ...
            starfile.write(df, output_file)
            logger.info(f"Saved name mappings for {len(name_data)} tomograms to {output_file}")
    ...
    def _convert_per_tilt_mapback(self, mapback_geom: Any, mapback_dir: Path) -> None:
        ...
        tilt_attrs = [attr for attr in dir(mapback_geom) 
                     if not attr.startswith('_') and 
                     attr not in ['tomoCoords', 'tomoName'] and
                     hasattr(mapback_geom, attr)]
    ...
    def _convert_raw_align(self, raw_align: Any, cycle_dir: Path) -> None:
        ...
        if all_align_data:
            ...
            starfile.write(combined_df, output_file)
            logger.info(f"Saved RawAlign data for {len(all_align_data)} tomograms to {output_file}")
    ...
    def _convert_other_cycle_data(self, cycle_data: Any, cycle_dir: Path) -> None:
        ...
        cycle_fields = [attr for attr in dir(cycle_data) 
                       if not attr.startswith('_') and 
                       attr not in ['geometry', 'Avg_geometry', 'RawAlign']]
