...
class EmClarityGeometryAnalyzer:
    ...
    def _load_star_metadata(self):
        ...
        if tilt_dir.exists():
            for tilt_file in tilt_dir.glob("*.star"):
                ...
                data_cols = [col for col in tilt_df.columns 
    ...
    def _load_matlab_metadata(self):
        ...
        mat_data = scipy.io.loadmat(str(self.metadata_source), struct_as_record=False, squeeze_me=True)
                    ...
                    for tomo_attr in dir(raw_align):
                        if not tomo_attr.startswith('_'):
                            ...
                            if isinstance(tomo_data, np.ndarray):
                                self.geometry_data[f"{cycle_name}_geometry"][tomo_attr] = tomo_data
    ...
    def get_geometry_data(self, cycle: Optional[str] = None, tomogram: Optional[str] = None) -> np.ndarray:
        """
        ...
        if cycle not in self.geometry_data:
            raise ValueError(f"Cycle '{cycle}' not found. Available: {list(self.geometry_data.keys())}")
    ...
    def create_tomogram_summary_plot(self, cycle: Optional[str] = None, 
        ...
        if len(included_data) > 0:
            ...
            ax1.hist(ccc_values, bins=max(10, len(ccc_values)//20), alpha=0.7, color='skyblue', edgecolor='black')
            ax1.axvline(np.mean(ccc_values), color='red', linestyle='--', label=f'Mean: {np.mean(ccc_values):.3f}')
            ax1.axvline(np.median(ccc_values), color='orange', linestyle='--', label=f'Median: {np.median(ccc_values):.3f}')
            ax1.set_xlabel('Cross-Correlation Coefficient')
            ax1.set_ylabel('Number of Particles')
            ax1.set_title(f'CCC Distribution\n{tomogram}\n{len(included_data)}/{len(geom_data)} particles')
            ax1.legend()
            ax1.grid(True, alpha=0.3)
        else:
            ax1.text(0.5, 0.5, 'No included particles', ha='center', va='center', transform=ax1.transAxes)
        ...
        if len(included_data) > 0:
            ...
            scatter = ax2.scatter(x_coords, y_coords, z_coords, c=ccc_values, 
        ...
        if len(included_data) > 0:
            ...
            ax3.bar(x_pos, odd_counts, width, label='ODD', alpha=0.7, color='lightcoral')
            ax3.bar(x_pos, eve_counts, width, bottom=odd_counts, label='EVE', alpha=0.7, color='lightblue')
            ...
            ax3.text(0.02, 0.98, f'Total ODD: {total_odd}\nTotal EVE: {total_eve}', 
                    transform=ax3.transAxes, va='top', ha='left',
                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
        else:
            ax3.text(0.5, 0.5, 'No class data', ha='center', va='center', transform=ax3.transAxes)
        ...
        if tilt_data is not None and len(tilt_data) > 0:
            ...
            if tilt_data.shape[1] > 1:
                ...
                # Plot tilt scheme
                line1 = ax4.plot(range(len(tilt_angles)), tilt_angles, 'bo-', label='Tilt Angle', markersize=4)
                line2 = ax4_twin.plot(range(len(doses)), doses, 'ro-', label='Cumulative Dose', markersize=4)
                ...
                ax4.text(0.02, 0.98, f'Range: {tilt_range}\nStep: {tilt_step:.1f}°', 
            ...
            ax4.text(0.5, 0.5, 'No tilt data available', ha='center', va='center', transform=ax4.transAxes)
            ax4.set_title('Tilt Series Analysis')
        
        # Add overall title
        fig.suptitle(f'emClarity Tomogram Analysis\nCycle: {cycle} | Tomogram: {tomogram}', 
    ...
    def create_cycle_overview_plots(self, cycle: Optional[str] = None,
                                  output_dir: Optional[Union[str, Path]] = None,
                                  max_tomograms: int = 20) -> None:
        """
        ...
        logger.info(f"Creating plots for {len(tomos_to_plot)} tomograms in cycle '{cycle}'")
    ...
    def _create_cycle_statistics(self, cycle: str, output_dir: Path) -> None:
            ...
            try:
                ...
                if len(included_data) > 0:
                    ...
                    stats_entry = {
                        ...
                        'inclusion_rate': len(included_data) / len(geom_data) if len(geom_data) > 0 else 0,
        ...
        if stats_data:
            ...
            with open(report_path, 'w') as f:
                ...
                f.write(f"  Included particles: {stats_df['included_particles'].sum()}\n")
                f.write(f"  Overall inclusion rate: {stats_df['included_particles'].sum() / stats_df['total_particles'].sum():.3f}\n\n")
                
                f.write(f"CCC Statistics:\n")
                f.write(f"  Mean CCC: {stats_df['mean_ccc'].mean():.4f} ± {stats_df['mean_ccc'].std():.4f}\n")
                f.write(f"  CCC range: {stats_df['min_ccc'].min():.4f} to {stats_df['max_ccc'].max():.4f}\n\n")
                ...
                f.write(f"  ODD/EVE ratio: {stats_df['n_odd'].sum() / max(stats_df['n_eve'].sum(), 1):.3f}\n\n")
                
                f.write(f"Per-tomogram Statistics:\n")
                f.write(f"  Particles per tomogram: {stats_df['included_particles'].mean():.1f} ± {stats_df['included_particles'].std():.1f}\n")
                f.write(f"  Classes per tomogram: {stats_df['n_classes'].mean():.1f} ± {stats_df['n_classes'].std():.1f}\n")
...
Examples:
  ...
  python emc_geometry_analysis.py project_star/ --list
    ...
    parser.add_argument('metadata_source', help='Path to metadata (star directory or .mat file)')
    ...
    parser.add_argument('--overview', action='store_true', help='Create overview plots for entire cycle')
    parser.add_argument('--list', action='store_true', help='List available cycles and tomograms')
    parser.add_argument('--output', help='Output directory for plots')
    parser.add_argument('--max-tomos', type=int, default=20, help='Maximum tomograms for overview (default: 20)')
        ...
        if args.list:
            ...
            if analyzer.current_cycle:
                ...
                if len(analyzer.list_available_tomograms()) > 10:
                    print(f"  ... and {len(analyzer.list_available_tomograms()) - 10} more")
